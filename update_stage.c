/**
 * Note for the Whole-Sun convection benchmark, modifications happen at lines 227-248
 */
...
*/
    /* ----------------------------------------------------
      ...
      // 2c.2 - Flux correction at the top and bottom of the domain
      //        Important note : We leave the pressure in the flux as given by the Riemann solver !
      if (g_dir == KDIR) {
        // Top of the domain : j==JBEG-1
        if (sweep.flag[KBEG-1] & FLAG_BOUNDARY) {
          double gval = g_inputParam[THETA] * g_inputParam[MPOLY]+1.0;

          sweep.flux[KBEG-1][RHO] = 0.0;
          sweep.flux[KBEG-1][MXt] = 0.0;
          sweep.flux[KBEG-1][MXn] = 0.0;
          sweep.flux[KBEG-1][MXb] = 0.0;
          sweep.flux[KBEG-1][ENG] = 0.0;
        }
        // Bottom of the domain : j==JEND
        else if (sweep.flag[KEND] & FLAG_BOUNDARY) {
          double gval = g_inputParam[THETA] * g_inputParam[MPOLY]+1.0;

          sweep.flux[KEND][RHO] = 0.0;
          sweep.flux[KEND][MXt] = 0.0;
          sweep.flux[KEND][MXn] = 0.0;
          sweep.flux[KEND][MXb] = 0.0;
          sweep.flux[KEND][ENG] = 0.0;
        }
      }
